...
final class Gemini_Provider implements Module, Translation_Provider {
	...
	public static function save_settings() {
		if ( ! current_user_can( 'manage_options' ) ) { wp_die( esc_html__( 'Permission denied.', 'openlingua' ) ); } check_admin_referer( 'openlingua_save_gemini' ); $s = self::settings(); $encrypted = ! empty( $_POST['clear_api_key'] ) ? '' : $s['api_key']; $key = trim( (string) wp_unslash( $_POST['api_key'] ?? '' ) ); if ( '' !== $key ) { $encrypted = self::encrypt_secret( $key ); }
	...
	public function translate( array $segments, $source_language, $target_language, array $context = array() ) {
		...
		foreach ( array_chunk( $segments, 40, true ) as $batch ) { $properties = array(); foreach ( $batch as $id => $value ) { $properties[ $id ] = array( 'type' => 'STRING' ); }
