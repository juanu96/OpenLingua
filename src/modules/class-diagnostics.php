...
final class Diagnostics implements Module {
	...
	public static function page() {
		...
		$checks = array(
			...
			array( 'status' => $report['languages'] > 1 ? 'good' : 'warning', 'title' => __( 'Site languages', 'openlingua' ), 'message' => $report['languages'] > 1 ? sprintf( _n( '%d language is enabled.', '%d languages are enabled.', $report['languages'], 'openlingua' ), $report['languages'] ) : __( 'Only one language is enabled. Add another language before creating translations.', 'openlingua' ), 'action' => admin_url( 'admin.php?page=openlingua' ), 'action_label' => __( 'Manage languages', 'openlingua' ) ),
			array( 'status' => $configured ? 'good' : 'info', 'title' => __( 'Automatic translation', 'openlingua' ), 'message' => $configured ? sprintf( _n( '%d translation service is configured.', '%d translation services are configured.', count( $configured ), 'openlingua' ), count( $configured ) ) : __( 'No automatic translation service is configured. Manual translation will continue to work normally.', 'openlingua' ), 'action' => admin_url( 'admin.php?page=openlingua-fields' ), 'action_label' => __( 'Configure services', 'openlingua' ) ),
		...
		echo '<div class="openlingua-health-summary openlingua-health-summary--' . ( $attention ? 'attention' : 'good' ) . '"><span class="dashicons ' . ( $attention ? 'dashicons-warning' : 'dashicons-yes-alt' ) . '" aria-hidden="true"></span><div><h2>' . esc_html( $attention ? __( 'Some items need your attention', 'openlingua' ) : __( 'OpenLingua is ready', 'openlingua' ) ) . '</h2><p>' . esc_html( $attention ? sprintf( _n( '%d item below may require action.', '%d items below may require action.', $attention, 'openlingua' ), $attention ) : __( 'The main translation components are working correctly.', 'openlingua' ) ) . '</p></div></div>';
