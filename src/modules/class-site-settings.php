...
final class Site_Settings implements Module {
	...
	private static function seo_section( $s ) {
		echo '<section class="openlingua-card"><h2>' . esc_html__( 'SEO, notifications, and permissions', 'openlingua' ) . '</h2>' . self::checkbox( 'noindex_incomplete', $s['noindex_incomplete'], __( 'Add noindex to incomplete translations', 'openlingua' ) ) . self::checkbox( 'noindex_fallback', $s['noindex_fallback'], __( 'Add noindex when fallback content is displayed', 'openlingua' ) ) . self::checkbox( 'notify_source_changes', $s['notify_source_changes'], __( 'Notify administrators when source content makes translations outdated', 'openlingua' ) ) . '<h3>' . esc_html__( 'Roles allowed to translate', 'openlingua' ) . '</h3>';
