...
final class Site_Settings implements Module {
	...
	public static function setup_page() {
		...
		foreach ( $languages as $code => $language ) { $search = strtolower( $code . ' ' . $language['name'] . ' ' . ( $language['native_name'] ?? '' ) ); echo '<label data-language-option data-search="' . esc_attr( $search ) . '"><input type="checkbox" name="enabled_languages[]" value="' . esc_attr( $code ) . '" data-flag="' . esc_attr( $language['flag'] ?? '🌐' ) . '" data-native="' . esc_attr( $language['native_name'] ?? $language['name'] ) . '" data-english="' . esc_attr( $language['name'] ) . '" ' . checked( isset( $enabled[ $code ] ), true, false ) . '><span>' . esc_html( $language['flag'] ?? '🌐' ) . '</span><strong>' . esc_html( $language['native_name'] ?? $language['name'] ) . '</strong><small>' . esc_html( $language['name'] ) . '</small><i class="dashicons dashicons-yes-alt" aria-hidden="true"></i></label>'; }
