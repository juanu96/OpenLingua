...
Current plugin version: **1.8.0**. Requires WordPress 6.4 or newer and PHP 7.4 or newer.

OpenLingua includes a local translation memory. Exact translations that you save manually or generate with an automatic provider can fill matching empty fields on the same page and on other translated content. Existing translations are never overwritten, and plain text is kept separate from HTML-compatible content.
